...
8. **去极值处理**：必须使用 MAD（Median Absolute Deviation）方法去极值，具体步骤：
   - 计算因子值的中位数和 MAD
   - 将超过 median ± 3×MAD 的值截断到边界值
   - 说明为什么选择 MAD 而非 3σ 或其他方法
9. **市值中性化处理**：必须进行市值中性化，具体步骤：
   - 使用对数市值（ln(总市值)）作为回归变量
   - 对因子值进行横截面回归：factor = α + β×ln(market_cap) + ε
   - 使用残差 ε 作为中性化后的因子值
   - 说明市值中性化的目的（消除市值暴露，纯化因子信号）
...
| G-DS-2 | 含"变量定义表"+"伪代码"+"未来函数三查"+"NaN 规则"+"单元测试"+"去极值"+"市值中性化"七个章节 |
| G-DS-3 | 未来函数三查勾选全部 ✓（任何一项未勾必 FAIL） |
| G-DS-4 | 至少 3 个单元测试用例，每个含输入样本 + 期望输出 |
| G-DS-5 | 列出的 common 函数全部存在（grep common/*.py 验证） |
| G-DS-6 | 去极值方法明确指定为 MAD（Median Absolute Deviation），且说明了参数（如 3×MAD） |
| G-DS-7 | 市值中性化方法明确指定（使用对数市值回归取残差），且说明了目的 |
